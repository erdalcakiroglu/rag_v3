...
| 2026-08-04 | 1.15 | **vLLM generation-migration MİLESTONE tanımlandı — fizibilite FAVORABLE, sert blokör YOK; ama config-flip DEĞİL milestone. Serve/kod/deploy YAPILMADI, tetikleyici konuldu.** A kolunun ([[kapasite-seri-lock]]) tek gerçek leveri (Ollama MoE hard-cap'ini vLLM continuous-batching çözer). Salt-okur ön-veri (konteyner + web) tüm sert kapıları YEŞİL buldu: **(pivotal) vLLM arch desteği ✓** — vLLM `Qwen3_5MoeForCausalLM` (text-only) + `Qwen3_5MoeForConditionalGeneration` destekliyor, resmi recipe var; RAG yalnız metin → text-only path yeter, vision taşınmaz (model = `qwen35moe` **35B-A3B**, 3B aktif, 256k ctx, Apache-2.0). **GPU** H200 Hopper 143GB / driver 570 / cc 9.0 ✓; **disk** 745G ✓; **lisans** Apache-2.0 → HF safetensors serbest ✓; **LiteLLM config-flip** ✓ (provider/api_base/api_key/model hepsi ENV, agent grafiği yalnız `LLMResponse` tüketir — geçen oturum gateway.py/settings.py audit'i). **AMA config-flip DEĞİL — milestone:** (1) greenfield vLLM standup (host'ta kurulu değil/image yok), (2) HF safetensors indir (vLLM'de GGUF-MoE zayıf → Q4_K_M'den **farklı quant substratı**), (3) **KALİTE RE-BASELINE (asıl pivot):** bugünkü mühürlü karneler Q4_K_M/Ollama'da ölçüldü → **D4 honesty 15/15 + rerank yönleri + M-16 mühürlü coverage/grounding** farklı quantta yeniden yeşil olmalı (§1c kalite-özdeşlik kapısı), (4) küçük gateway re-wire (thinking `chat_template_kwargs`, timing native-ns → zarifçe latency_ms'e düşer, token `usage` hayatta kalır). **İKİ bağımsız değer akışı:** (i) eşzamanlılık (MoE continuous-batching, ADR-003 3-9×); (ii) **gs-012 (C) guided_json** kalıcı kapanışı vLLM'e bağlıydı ([[gs012-toolcall-json-bug]]) → pertürbasyon-retry heuristiğini gereksiz kılar. **Dürüst talep-uyarısı (#11/A disiplini):** çözeceği eşzamanlılık **33-turlik genç logdan kanıtlanamıyor**; seri tavan ~4-5 istek/dk incitiyor mu ölçülemez → milestone-ölçekli maliyet + kanıtlanmamış talep, kalıcı infra ERKEN ([[on-veri-kontrolu-kural]]). **GO-TRIGGER:** (T1) log ≥~150-200 tur olgunlaşınca eşzamanlı kuyruk ölçülebilir gecikme üretiyorsa; VEYA (T2) gs-012 parse-error/pertürbasyon-retry canlıda sıklaşırsa. **TETİKLEYİCİ ÇERÇEVE NETLEŞTİRMESİ (2026-08-04 canlı sayım):** T1'in ham "≥150-200 tur" eşiği bir **PROXY**'dir; gerçek ön-koşul **gerçek kullanıcı adopsiyonu / üretim canlıya açılışı**. Canlı log bugün **2 kullanıcı / 33 user-turu / 13 sohbet, 2026-07-20→08-03, v1.13'ten beri DONMUŞ** (= dev/demo trafiği, organik talep değil) → ham sayı bu bağlamda **fiilen hiç tetiklenmeyebilir**. Tetikleyici bu yüzden *"gerçek kullanıcılar onboard olunca / üretim açılınca prevalence+kuyruk probu'nu tekrar koş"* olarak okunur; sayı bunun proxy'sidir. **AYNI ÇERÇEVE #11 için de geçerli** ([[rerank-ab-onveri]] tetikleyicisi de adopsiyon-koşullu). **Tetiklenirse İLK adım (kalıcı deploy DEĞİL):** tek trial serve (text-only Qwen3.5-MoE FP8/AWQ) + bind-probe (gerçekten paralelleşiyor mu? Ollama'da oran 1.94'tü, ~1.0 beklenir) + mini-karne (Q4_K_M'e karşı kalite deltası). **Kapsam=generation-only; embedder Ollama'da KALIR** (bge-m3 non-MoE, `bge-m3@ollama` damgası 1478 vektöre gömülü, `assert_corpus_model` invaryantı). **Verdict: fizibilite favorable, sert blokör yok; milestone-ölçekli (standup + ağırlık + tam re-baseline), talep kanıtlanana dek tetikleyicili beklemede. Brief: `docs/Brief_vLLM_Generation_Migration.md`; değişen üretim kodu YOK.** [[vllm-generation-migration]] [[kapasite-seri-lock]] [[gs012-toolcall-json-bug]] [[on-veri-kontrolu-kural]] |
| 2026-08-03 | 1.14 | **A kolu (kapasite/eşzamanlılık) KAPANDI — KÖK KESİN: `qwen3.5:35b` bir MoE (`qwen35moe`); Ollama 0.17.4 bu mimaride paralel isteği KOD SEVİYESİNDE reddediyor → LLM decode sert-seri. Planlanan iki değişiklik de throughput'u kaldırmıyor. AKSİYON YOK.** 1.13 A'yı "sıradaki odak" bırakmıştı; onaylı 3-değişiklik planı (NUM_PARALLEL + checkpointer-pool + striped-lock) Adım-0 baseline ile başladı. **Adım-0 (canlı, `kapasite_bind_noktasi_probe.py`, EFFECTIVE_MODEL app.py:64 çözümü = `qwen3.5:35b`):** 2 eşzamanlı Ollama-direct → bireysel [3013ms, 5834ms], duvar 5835ms, **oran 1.94 ≈ 2.0 + pf 1.52 = tam seri imza** (bekle-sonra-çalış; baseline tek çağrı 13s = soğuk 34GB reload, warm üretim ~3s). **Change-1 denendi (canlı, reversible): systemd drop-in'e `OLLAMA_NUM_PARALLEL=4` + daemon-reload + restart; `systemctl show` env'de YÜKLÜ ✓.** Test A **bit-bit aynı** (oran 1.94, pf 1.52) → hipotez KANITLA ÇÜRÜDÜ (kilide dokunmadan, ucuz). **KÖK — yükleme logu kesin:** `journalctl -u ollama` → `sched.go:452 WARN "model architecture does not currently support parallel requests" architecture=qwen35moe`; üç load request'te de `Parallel:1` (env=4 YOK SAYILDI). `ollama ps`: 34 GB / 100% GPU / ctx 262144 resident. **1.10'un DÜZELTMESİ:** "serileşmenin kökü `NUM_PARALLEL=1` config" demiştim — IMPRECISE. Gerçek kök **config değil MİMARİ hard-cap**: env açıkça 4 verilse bile Ollama MoE'yi 1'e zorluyor. VRAM (143 GB boş) tümüyle alâkasız. **Change-1 GERİ ALINDI** (`sed -i /OLLAMA_NUM_PARALLEL=4/d` + restart; MoE'de inert → config'i dürüst tut). **Change-2 (API `_lock` kaldırma + pool'lu checkpointer) YAPILMADI:** LLM decode zaten Ollama'da sert-seri olduğundan kilit-kaldırma **LLM-bağlı** throughput'u çarpmaz (yalnız LLM-dışı fazları örtüştürür + kuyruğu Ollama'nın adil kuyruğuna taşır) — marjinal ödül vs **M-16 mühürlü checkpointer yolunu elleme riski** → değmez. **GERÇEK LEVER = ADR-003 (Prod=vLLM, 3-9× throughput; MoE continuous batching native)** — zaten kabul edilmiş mimari yön, uygulanmamış; ya da `OLLAMA_NEW_ENGINE=true` (ollamarunner MoE'yi batch'leyebilir ama üretim kalite/davranış re-validation'ı şart, bedava değil). İkisi de bu oturumun kapsamı dışı, ayrı ön-veri. **Verdict: kapasite tavanı artık precise root-cause'lu — tek-GPU'da değil, Ollama'nın MoE'yi paralelleştirmemesi; ne NUM_PARALLEL ne kilit-kaldırma kaldırır. Değişen üretim kodu YOK (config geçici eklendi + geri alındı).** [[kapasite-seri-lock]] [[on-veri-kontrolu-kural]] [[olcum-zemini-dersleri]] |
| 2026-08-03 | 1.13 | **Görev #11 ASKIYA ALINDI (reddedilmedi) — prevalans ön-verisi: korpus tablo-ağırlıklı (arz güçlü) ama canlı log rerank-ilgili talebi KANITLAYAMIYOR (genç). Tetikleyici konuldu; sıradaki odak A (kapasite). Kod/TEI infra YOK.** 1.12 gate'i "yalnız table_based" daraltmıştı; kalan değer-sorusu: tablo dilimi anlamlı mı ki kalıcı kod yolu + TEI serving bağımlılığı kurulsun? İki salt-okunur proxy konteynerde koşuldu (`tablo_prevalence_probe.py`, prod değişmez). **ARZ güçlü:** chunk'ların **%21.3'ü tablo-kökenli** (315/1478, `core_chunks.table_id` M-2b kalıcı işareti), **184 tablo, 41 dosyanın 27'sinde** → tablo-ağırlıklı kurumsal-rapor korpusu; tablo-cevap sorusu geldiğinde içerik bol + sıralama problemi gerçek (+0.400 buna değer). **TALEP ölçülemez:** yalnız **33 kullanıcı turu** (`conversation_messages` role=user; M-13 yeni → genç log, DÜŞÜK talep değil). 3/33 "tablo" geçiyor; **regex %0 ölçü-aracı kusurum** ("tablosuna"/çıplak "tablo N" kaçtı, güvenme). Örneklerin gözle okuması iki şey söyledi: (a) **yanlış-pozitif riski düşük** — 3/3 gerçek tablo-niyeti, 0 gizli multi_hop (senin işaret ettiğin gevşek-kullanım riskine iyi haber, küçük örnek); (b) **kritik nüans** — 2/3 tablo-GÖRÜNTÜLEME ("tablo 4'ü görebilir miyim" → M-2 view yolu, rerank'e DEĞİL), yalnız 1 rerank-cevap sorusu → rerank'in yardım ettiği dilim ham "tablo" oranından daha ince. **KARAR: #11 ASKIYA ALINDI.** Korpus yeteneği ilkesel hakediyor ama canlı log rerank-ilgili talebi kanıtlayamıyor → ölçülmemiş premise üstüne kalıcı kod + TEI infra kurma ([[on-veri-kontrolu-kural]]). **TETİKLEYİCİ:** log ≥~150-200 tura olgunlaşınca aynı probe TEKRAR koşulur; tablo-CEVAP dilimi ("tablosuna göre X kaç") gerçek çıkarsa greenlight — brief hazır (muhafazakâr "tablo-niyet" dedektörü + ÜRETİM-tarzı gate-isabet/yanlış-pozitif eval + TEI-CPU; GPU ertelendi). Bu arada **A kolu ([[kapasite-seri-lock]]) daha yüksek kaldıraç** (eşzamanlılık TÜM kullanıcıları etkiler, tablo alt-küme) → sıradaki. **Verdict: #11 fizibilite kanıtlı, talep ölçülemediği için askıda; değişen üretim kodu YOK.** [[on-veri-kontrolu-kural]] [[olcum-zemini-dersleri]] |
